<p align="center">
  <img src="logo.png" alt="SpectreHunt" width="280"/>
</p>

<h1 align="center">SpectreHunt</h1>

<p align="center">
  <b>HackerOne novelty hunter — fresh surface, dedup gates, logic flaws.</b><br>
  <sub>Enhanced fork of <a href="https://github.com/shuvonsec/claude-bug-bounty">claude-bug-bounty</a> · recon → validate → report</sub>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/HackerOne-Novelty%20Hunt-1f6feb?style=flat-square" alt="HackerOne Novelty Hunt">
...
## SpectreHunt additions (vs upstream)

| Command | Purpose |
|:---|:---|
| `bughunter watch scan --min-bounty 50000` | H1 programs with fresh scope / bounty changes |
| `bughunter hunt target.com --novelty-first --program HANDLE` | Fresh surface + logic hunter (not mass nuclei) |
| `bughunter novelty --program HANDLE --endpoint /path --vuln-class idor` | Public Hacktivity dedup + novelty score |
| `python3 tools/validate.py --auto-dedup ...` | Block obvious duplicate reports |
| `python3 tools/surface_diff.py --target domain.com` | New URLs vs Wayback history |
| `python3 tools/logic_hunter.py --target domain.com --program HANDLE` | IDOR/auth/mass-assignment candidates |

**Push to GitHub:** create any empty repo, push, then rename it in GitHub Settings if you like:

```bash
./scripts/push_to_github.sh tgollogly YOUR-REPO-NAME
# Rename later: GitHub → Settings → General → Repository name
# Make private: Settings → General → Change visibility
```

**Quick start after clone:**

```bash
./install.sh --agent standalone   # installs bughunter + spectre commands
./install_tools.sh
bughunter watch scan --min-bounty 50000
bughunter hunt target.com --novelty-first --program HACKERONE_HANDLE
```

---

<p align="center"><sub>Original BugHunt readme continues below</sub></p>
