...
install_standalone() {
    echo ""
    echo "════════════════════════════════════════════════════"
    echo "  SpectreHunt Standalone Engine (no subscription)"
    echo "════════════════════════════════════════════════════"
    echo ""
    ...
    SPECTRE="$BIN_DIR/spectre"

    # Remove old symlink/file if exists
    $SUDO rm -f "$TARGET" "$SPECTRE" 2>/dev/null || true
    $SUDO ln -sf "$ENGINE" "$TARGET"
    $SUDO ln -sf "$ENGINE" "$SPECTRE"

    if [ -f "$TARGET" ] || [ -L "$TARGET" ]; then
        echo "[+] Installed: bughunter -> $TARGET"
        echo "[+] Installed: spectre   -> $SPECTRE (alias)"
    else
        ...
        echo "    alias spectre='python3 $ENGINE'"
    fi
    ...
    echo ""
    echo "    bughunter setup          (or: spectre setup)"
    echo "    bughunter watch scan --min-bounty 50000"
    echo "    bughunter hunt target.com --novelty-first --program HANDLE"
    echo "════════════════════════════════════════════════════"
    echo ""
}
