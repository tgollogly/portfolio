...
export function buildWeatherAlerts(days, current = null) {
  ...
  for (const d of days) {
    ...
    if (maxT >= 28) {
      add({ type: "heat", severity: "high", date: d.date, label: d.label, title: "Heat alert", message: `${d.label}: ${formatTempC(maxT)} — hot sand, walk early or late` });
    } else if (maxT >= 25) {
      add({ type: "heat", severity: "moderate", date: d.date, label: d.label, title: "Warm spell", message: `${d.label}: ${formatTempC(maxT)} — shade & water for Max` });
    }
    if (uv >= 8) {
...
export function buildDailyRows(payload, now = new Date()) {
...
export function buildForecastResponse(payload, meta = {}) {
  ...
  const timeZone = payload?.timezone || BETTYSTOWN.timezone;
  const daily = buildDailyRows(payload, now);
