...
export const BETTYSTOWN = {
  ...
  county: "County Meath",
  country: "Ireland",
  /** Fixed pin for Mom & Max (Bettystown beach, County Meath). */
  latitude: 53.698,
  longitude: -6.248,
...
export const RADAR_REFRESH_TTL_SEC = 90;
export const RADAR_PLAY_FRAME_COUNT = 6;
/** Show green “Live” only when the newest frame is younger than this (minutes). */
export const RADAR_LIVE_THRESHOLD_MIN = 5;

/**
 * Geographic bounds of Met Éireann 12h rainfall radar PNG (full frame:
 * Ireland, Irish Sea, and west Britain — not Ireland-only).
 */
export const MET_RADAR_MAP_BOUNDS = {
  west: -10.75,
  east: -3.05,
  south: 51.25,
  north: 55.45,
...
/** On Met Éireann composite, further east than this is usually Irish Sea (not land). */
export const RADAR_EAST_COAST_LAND_MAX_LEFT = 67;

/** Full Ireland in frame (no east-coast crop). */
export const BETTYSTOWN_RADAR_VIEW = { zoom: 1 };

/**
 * Met Éireann PNG vs WGS84 linear bounds — calibrated with OSM overlay
 * (see scripts/calibrate-bettystown-radar.mjs). Not shown in production UI.
 */
export const MET_RADAR_GEO_CALIBRATION = { leftOffset: 4.59, topOffset: -1.41 };
...
export function explainWalkDay(day, walk = null) {
  ...
  const dayOffset = bands?.dayOffset ?? day.dayOffset ?? null;
  const skipLastNight =
    day.relative === "today" &&
    (dayOffset === 0 || dayOffset == null) &&
    hourInTimeZone(new Date(), BETTYSTOWN.timezone) >= 8;
  if (bands && overnightMm >= 1.5 && daytimeMm < 1 && !skipLastNight) {
    positives.push({
      key: "overnight-rain",
...
/** Plain 12-hour clock label for Mom (e.g. 3pm). */
export function formatHour12Irish(hour24) {
...
/** Hour-by-hour outlook for the current calendar day (Irish time). */
export function buildTodayHourlyOutlook(hourlyRows, now = new Date(), timeZone = BETTYSTOWN.timezone) {
  const ymd = ymdInTimeZone(now, timeZone);
  const meta = formatDayMeta(ymd, now, timeZone);
  const currentHour = hourInTimeZone(now, timeZone);
  const hours = hoursForCalendarDay(hourlyRows, ymd)
    .filter((r) => {
      const h = Number(r.time.slice(11, 13));
      return h >= 6 && h <= 22;
    })
    .map((r) => {
      const h = Number(r.time.slice(11, 13));
      const mm = Number(r.precipitation ?? 0);
      const gust = Number(r.wind_gusts_10m ?? r.wind_speed_10m ?? 0);
      const walkLabel = maxWalkBandLabel(mm, gust);
      const code = Number(r.weather_code ?? 0);
      const wmo = wmoInfo(code);
      const temp = Number(r.temperature_2m);
      return {
        hour: h,
        timeLabel: formatHour12Irish(h),
        ...
        summary: wmo.label,
        walkLabel,
        walkClass: walkLabel === "Good" ? "good" : walkLabel === "Watch" ? "watch" : "wet",
  ...
  return {
    ...
    label: meta.relative === "today" ? "Today" : meta.dateShort,
    hours,
  };
}
...
export function buildWeatherAlerts(days, current = null, ctx = {}) {
  ...
  for (const d of days) {
    ...
    if (headline && dayTotal > 3) {
      ...
      let skipRainCard =
        overnightOnly && hideOvernightPastForToday(d.relative, dayOffset, now, timeZone);
      if (!skipRainCard && overnightOnly && dayOffset > 3) {
        farOvernightShown += 1;
        if (farOvernightShown > 2) skipRainCard = true;
      }
      if (!skipRainCard) {
  ...
  if (current) {
    ...
    if (curWind >= 45) {
      add({
        ...
        label: "Now",
        title: "Windy now",
        message: `Currently ${Math.round(curWind)} km/h — hold Max's lead tight`,
        confidence: "Based on right-now readings",
      });
    }
  }
...
/** Local hour 0–23 in Europe/Dublin (for walk-day / overnight cutoffs). */
export function hourInTimeZone(date = new Date(), timeZone = BETTYSTOWN.timezone) {
...
/** After 8am Irish, “last night’s rain” for today is no longer shown. */
export function hideOvernightPastForToday(dayRelative, dayOffset, now = new Date(), timeZone = BETTYSTOWN.timezone) {
  return dayRelative === "today" && dayOffset === 0 && hourInTimeZone(now, timeZone) >= 8;
}
...
export function buildForecastResponse(payload, meta = {}) {
  ...
  return {
    ...
    metEireann: metBrief?.ok
      ...
      : null,
    maxWalkScore,
    todayHourly: buildTodayHourlyOutlook(hourlyRows, now, timeZone),
    ...
    stale,
    staleHint: stale ? "Forecast may be older than 2 hours — refresh the page for the latest." : null,
...
export function validateForecastResponse(body) {
  ...
  if (!body?.todayHourly?.hours?.length) issues.push("no todayHourly");
...
export function formatRadarFrameIrishLabel(filename) {
  ...
  return `${Number(d)} ${month} · ${Number(h)}:${mi} Irish time`;
}

export function latLonToRadarPinPct(
  lat,
  lon,
  bounds = MET_RADAR_MAP_BOUNDS,
) {
  const leftPct = ((lon - bounds.west) / (bounds.east - bounds.west)) * 100;
  const topPct = ((bounds.north - lat) / (bounds.north - bounds.south)) * 100;
  return {
    leftPct: Math.min(97, Math.max(3, leftPct)),
    topPct: Math.min(97, Math.max(3, topPct)),
  };
}

/**
 * Map town lat/lon to radar pixels and keep terrestrial pins off the sea
 * (east-coast Ireland: nudge west if the point falls in the Irish Sea).
 */
export function applyMetRadarGeoCalibration(computed) {
  let leftPct = computed.leftPct + MET_RADAR_GEO_CALIBRATION.leftOffset;
  let topPct = computed.topPct + MET_RADAR_GEO_CALIBRATION.topOffset;
  let onLand = true;
  if (leftPct > RADAR_EAST_COAST_LAND_MAX_LEFT) {
    leftPct = RADAR_EAST_COAST_LAND_MAX_LEFT - 0.35;
    onLand = false;
  }
  leftPct = Math.min(RADAR_EAST_COAST_LAND_MAX_LEFT, Math.max(3, leftPct));
  topPct = Math.min(97, Math.max(3, topPct));
  return { leftPct, topPct, onLand };
}

/** @deprecated Use applyMetRadarGeoCalibration — kept for tests and callers. */
export function resolveTerrestrialRadarPin(lat, lon) {
  const computed = latLonToRadarPinPct(lat, lon);
  const { leftPct, topPct, onLand } = applyMetRadarGeoCalibration(computed);
  return { leftPct, topPct, onLand, computedLeftPct: computed.leftPct, computedTopPct: computed.topPct };
}

export function bettystownRadarPin() {
  const computed = latLonToRadarPinPct(BETTYSTOWN.latitude, BETTYSTOWN.longitude);
  const { leftPct, topPct, onLand } = applyMetRadarGeoCalibration(computed);
  return {
    label: BETTYSTOWN.name,
    lat: BETTYSTOWN.latitude,
    lon: BETTYSTOWN.longitude,
    leftPct,
    topPct,
    onLand,
    computedLeftPct: computed.leftPct,
    computedTopPct: computed.topPct,
    zoom: BETTYSTOWN_RADAR_VIEW.zoom,
  };
}

/** UTC instant for an Irish wall-clock time (Europe/Dublin). */
export function dateAtIrishWallClock(ymd, hour, minute, timeZone = BETTYSTOWN.timezone) {
  const [Y, M, D] = ymd.split("-").map(Number);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const diffMin = (hour - get("hour")) * 60 + (minute - get("minute"));
    if (diffMin === 0) return new Date(t);
    t += diffMin * 60_000;
  }
  return new Date(t);
}

export function radarFrameAsDate(filename, timeZone = BETTYSTOWN.timezone) {
  const m = filename.match(/_(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})\.png$/);
  if (!m) return null;
  const [, y, mo, d, h, mi] = m;
  return dateAtIrishWallClock(`${y}-${mo}-${d}`, Number(h), Number(mi), timeZone);
}

/** Whole minutes between frame timestamp (Irish, from filename) and now. */
export function radarFrameAgeMinutes(filename, now = new Date(), timeZone = BETTYSTOWN.timezone) {
  const frameAt = radarFrameAsDate(filename, timeZone);
  if (!frameAt) return null;
  return Math.max(0, Math.floor((now.getTime() - frameAt.getTime()) / 60_000));
}

export function formatRadarAgeMinutes(ageMin) {
  if (ageMin == null || !Number.isFinite(ageMin)) return "";
  if (ageMin < RADAR_LIVE_THRESHOLD_MIN) return `updated ${ageMin} min ago`;
  if (ageMin <= 120) return `Displaying latest available · ${ageMin} min old`;
  return "Displaying latest available radar from Met Éireann";
}

export function isRadarFeedLive(ageMin) {
  return ageMin != null && ageMin < RADAR_LIVE_THRESHOLD_MIN;
}
...
export function buildBettystownRadarResponse(frames, origin) {
  ...
  const latestAgeMinutes = latest?.file ? radarFrameAgeMinutes(latest.file) : null;
  const latestFrameAtMs = latest?.file ? radarFrameAsDate(latest.file)?.getTime() : null;
  return {
    ok: true,
    source: "Met Éireann",
    ...
    intervalMinutes: 15,
    pin: bettystownRadarPin(),
    frames: mapped,
    latest: latest
      ? { ...latest, frameAtMs: latestFrameAtMs ?? undefined }
      : null,
    latestAgeMinutes,
    radarLive: isRadarFeedLive(latestAgeMinutes),
  };
}
