/** Tom's Bettystown Weather — Open-Meteo forecast + dog-walk scoring (free, no API key). */
...
export function buildDailyRows(payload) {
  ...
  return rows.sort((a, b) => b.walkScore - a.walkScore || a.date.localeCompare(b.date));
}

export function buildForecastResponse(payload, meta = {}) {
  const daily = buildDailyRows(payload);
  const best = daily.filter((d) => d.walkTier === "excellent" || d.walkTier === "good").slice(0, 5);
  ...
  return {
    ...
    current: current
      ...
      : null,
    bestDays: best,
    forecast: daily.sort((a, b) => a.date.localeCompare(b.date)),
