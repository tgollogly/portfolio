...
export function maxWalkBandLabel(mm, gust) {
  ...
  if (mm >= 0.4 || gust >= 50) return "Watch";
  return "Good";
}
...
export function parseMetEireannJson(json) {
  ...
  const oneLiner =
    [out.outlook, out.tomorrow, out.today, out.tonight].find((t) => t && String(t).trim().length > 20) ||
    "";
  return {
    ...
    outlook: out.outlook || "",
    oneLiner: String(oneLiner).trim().slice(0, 280),
  };
}
...
export async function fetchMetEireannBrief(fetchFn = fetch) {
  ...
  try {
    ...
    const oneLiner =
      leinster?.outlook?.slice(0, 120) ||
      leinster?.tonight?.slice(0, 120) ||
      national?.outlook?.slice(0, 120) ||
      leinster?.oneLiner ||
      "";
    return {
      ok: true,
...
export function buildWeatherAlerts(days, current = null, ctx = {}) {
  ...
  for (const d of days) {
    ...
    const conf = formatConfidenceLine(dayOffset, "rain");
    ...
    if (headline && dayTotal > 3) {
      ...
      add({
        ...
        tier: bands.daytime.mm >= 8 ? "red" : bands.daytime.mm >= 2 ? "amber" : "green",
        date: d.date,
        label: dateTag,
        title: bands.daytime.mm < 2 && bands.overnight.mm >= 4 ? "Overnight rain" : "Rain timing",
        message: headline,
        bands,
        confidence: conf,
      });
    ...
    const dayHours = hoursForCalendarDay(hourlyRows, d.date);
    const roll6 = dayHours.length ? rolling6hRainMax(dayHours) : 0;
    const roll3 = dayHours.length ? rolling3hRainMax(dayHours) : 0;
    const daytimeMm = bands.daytime.mm;
    const daytimeGust = bands.daytime.maxGust;

    let walkTier = "green";
    if (daytimeMm >= 8 || roll6 >= 8) walkTier = "red";
    else if (daytimeMm >= 2 || daytimeGust >= 50 || roll6 >= 4) walkTier = "amber";

    if (walkTier === "green" && daytimeMm < 2 && daytimeGust < 50) {
      add({
        type: "walk_ok",
        severity: "low",
        tier: "green",
        ...
        label: dateTag,
        title: "Good for Max",
        message: `${dateTag} daytime (${bands.daytime.text}, gusts ~${Math.round(daytimeGust)} km/h) — ${conf}`,
        bands,
        confidence: conf,
      });
    } else if (walkTier === "amber") {
      add({
        type: "walk_watch",
        severity: "moderate",
        tier: "amber",
        ...
        label: dateTag,
        title: "Watch",
        message: `${dateTag}: daytime ${bands.daytime.text}, overnight ${bands.overnight.text}, gusts ~${Math.round(daytimeGust)} km/h — ${conf}`,
        bands,
        confidence: conf,
      });
    }

    const heavyFromMm = rainMm >= 8 || roll6 >= 8;
    const heavyBurst = roll3 >= 4;
    ...
    if ((heavyFromMm || heavyBurst) && !rainTimingAdded) {
      ...
      add({
        ...
        label: dateTag,
        title,
        message: `${dateTag}: overnight ${bands.overnight.text}, daytime ${bands.daytime.text} — ${conf}`,
        bands,
        confidence: conf,
      });
      ...
      add({
        ...
        message: `${dateTag}: short sharp burst possible (~${roll3.toFixed(1)} mm in 3 h) — ${conf}`,
        bands,
        confidence: conf,
      });
    ...
    if (STORM_CODES.has(code) && dayOffset <= 5) {
      add({
        ...
        message: `${dateTag}: thunderstorms in forecast — keep Max off open beach — ${conf}`,
        confidence: conf,
      });
    }

    const gustConf = formatConfidenceLine(dayOffset, "gust");
    if (gust >= 80 && dayOffset <= 2) {
      add({
        ...
        message: `${dateTag}: gusts to ${Math.round(gust)} km/h on exposed beach — ${gustConf}`,
        confidence: gustConf,
      });
      ...
      add({
        ...
        message: `${dateTag}: gusts to ${Math.round(gust || wind)} km/h — hold Max's lead — ${gustConf}`,
        confidence: gustConf,
      });
      ...
      add({
        ...
        message: `${dateTag}: models hint gusts ~${Math.round(gust)} km/h — not a Met Éireann warning — check again in a few days. ${gustConf}`,
        confidence: gustConf,
      });
    }

    if (maxT >= 28) {
      add({ type: "heat", severity: "high", tier: "amber", date: d.date, label: dateTag, title: "Heat alert", message: `${dateTag}: ${formatTempC(maxT)} — hot sand, walk early or late — ${conf}`, confidence: conf });
    } else if (maxT >= 25) {
      add({ type: "heat", severity: "moderate", tier: "amber", date: d.date, label: dateTag, title: "Warm spell", message: `${dateTag}: ${formatTempC(maxT)} — shade & water for Max — ${conf}`, confidence: conf });
    }
    if (uv >= 8) {
      add({ type: "uv", severity: "moderate", tier: "amber", date: d.date, label: dateTag, title: "High UV", message: `${dateTag}: UV ${uv.toFixed(0)} — limit midday sun — ${conf}`, confidence: conf });
    }
    if (minT <= 0) {
      add({ type: "frost", severity: "moderate", tier: "amber", date: d.date, label: dateTag, title: "Frost", message: `${dateTag}: freezing overnight — icy paths possible — ${conf}`, confidence: conf });
    }
  }
  ...
  const tierOrder = { red: 0, amber: 1, green: 2, grey: 3 };
...
export function buildForecastResponse(payload, meta = {}) {
  ...
  return {
    ...
    attribution:
      "Hourly & daily: Open-Meteo.com (CC BY 4.0) · Regional text: Met Éireann",
    current: currentOut,
...
export async function getBettystownForecast(env, fetchFn = fetch) {
  ...
  if (live.ok) {
    ...
    const response = buildForecastResponse(live.data, {
