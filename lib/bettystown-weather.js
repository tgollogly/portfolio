...
export function parseMetEireannJson(json) {
  ...
  const oneLiner = [out.today, out.tonight, out.tomorrow].filter(Boolean).join(" ").trim().slice(0, 280);
  return {
...
export function buildWeatherAlerts(days, current = null, ctx = {}) {
  ...
  for (const d of days) {
    ...
    } else if (dayOffset <= 4 && walkTier === "amber") {
      add({
        type: "walk_watch",
        severity: "moderate",
        tier: "amber",
        ...
        label: dateTag,
        title: "Watch the sky",
        message: `${dateTag}: daytime ${bands.daytime.text}, overnight ${bands.overnight.text}.`,
        bands,
        confidence: conf,
      });
    }
    ...
    if ((heavyFromMm || heavyBurst) && !rainTimingAdded) {
      ...
      add({
        ...
        message: `${dateTag}: a short sharp shower possible (~${roll3.toFixed(1)} mm in a few hours).`,
        bands,
        confidence: conf,
      });
