...
export const BETTYSTOWN_RADAR_VIEW = { zoom: 1.72 };

/** Fine-tune on Met Éireann 12h PNG after lat/lon → % (visual vs Boyne coast). */
export const BETTYSTOWN_RADAR_PIN_TUNE = { leftShift: -0.9, topShift: 1.05 };
...
export function resolveTerrestrialRadarPin(lat, lon) {
  ...
  let leftPct = computed.leftPct + 7.5;
  let topPct = computed.topPct - 0.8;
  let onLand = true;
  if (leftPct > RADAR_EAST_COAST_LAND_MAX_LEFT) {
    leftPct = RADAR_EAST_COAST_LAND_MAX_LEFT - 0.3;
    onLand = false;
  }
  leftPct = Math.min(RADAR_EAST_COAST_LAND_MAX_LEFT, Math.max(58, leftPct));
  topPct = Math.min(46, Math.max(37, topPct));
...
export function bettystownRadarPin() {
  ...
  let leftPct = resolved.leftPct + BETTYSTOWN_RADAR_PIN_TUNE.leftShift;
  let topPct = resolved.topPct + BETTYSTOWN_RADAR_PIN_TUNE.topShift;
  leftPct = Math.min(RADAR_EAST_COAST_LAND_MAX_LEFT, Math.max(58, leftPct));
  topPct = Math.min(46, Math.max(37, topPct));
  const onLand = leftPct <= RADAR_EAST_COAST_LAND_MAX_LEFT - 0.05;
  return {
    ...
    topPct,
    onLand,
    computedLeftPct: resolved.computedLeftPct,
    computedTopPct: resolved.computedTopPct,
