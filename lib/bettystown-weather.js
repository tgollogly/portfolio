...
export function formatDayMeta(isoDate, now = new Date()) {
  ...
  const dateShort = d.toLocaleDateString("en-IE", { weekday: "short", day: "numeric", month: "short" });
  const dateLong = d.toLocaleDateString("en-IE", { weekday: "long", day: "numeric", month: "long" });
  let label = dateShort;
  let relative = null;
  if (diff === 0) {
    label = "Today";
  ...
  return { label, dateShort, dateLong, relative, diff };
}
...
export function buildDailyRows(payload) {
  ...
  for (let i = 0; i < daily.time.length; i++) {
    const meta = formatDayMeta(daily.time[i]);
    const day = {
      date: daily.time[i],
      label: meta.label,
...
export function buildForecastResponse(payload, meta = {}) {
  ...
  return {
    ...
    location: BETTYSTOWN,
    today: todayRow
      ? { date: todayRow.date, dateLong: todayRow.dateLong, dateShort: todayRow.dateShort, label: todayRow.label }
      : null,
