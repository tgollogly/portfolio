...
/**
 * Hand-calibrated on Met Éireann 12h PNG: Irish east coast (Bettystown beach),
 * not inland and not on Wales/UK.
 */
export const BETTYSTOWN_RADAR_PIN = {
  leftPct: 68.2,
  topPct: 40.5,
  zoom: 1.72,
};
...
export function bettystownRadarPin() {
  ...
  return {
    ...
    leftPct: BETTYSTOWN_RADAR_PIN.leftPct,
    topPct: BETTYSTOWN_RADAR_PIN.topPct,
    zoom: BETTYSTOWN_RADAR_PIN.zoom,
  };
}

/** Minutes between frame timestamp (Irish wall clock in filename) and now. */
export function radarFrameAgeMinutes(filename, now = new Date(), timeZone = BETTYSTOWN.timezone) {
  const m = filename.match(/_(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})\.png$/);
  if (!m) return null;
  const [, y, mo, d, h, mi] = m;
  const frameDay = `${y}-${mo}-${d}`;
  const nowYmd = ymdInTimeZone(now, timeZone);
  const dayDiff = dayDiffYmd(frameDay, nowYmd);
  const frameMins = Number(h) * 60 + Number(mi);
  const nowH = hourInTimeZone(now, timeZone);
  const nowMin = Number(
    new Intl.DateTimeFormat("en-GB", { timeZone, minute: "numeric" }).format(now),
  );
  const nowMins = nowH * 60 + nowMin;
  if (dayDiff === 0) return Math.max(0, nowMins - frameMins);
  if (dayDiff === 1) return nowMins + (24 * 60 - frameMins);
  return Math.max(0, dayDiff * 24 * 60 + nowMins - frameMins);
}
