...
const EXFILTRATION_PATTERNS = [
  ...
  /\.vcf\b/i,
  /\b(his|her|their|the)\s+(mobile|phone|cell|whatsapp)\b/i,
  /\b(give|show|tell|share|send)\s+(me\s+)?(his|her|their)\s+(mobile|phone|cell)\b/i,
  /\bextract\b[\s\S]{0,40}\b(phone|mobile|number|contact)\b/i,
  /\b(phone|mobile|cell|telephone|whatsapp)\s*(number|#|no\.?|details?)\b/i,
  /\b(list|listing|directory|contents?)\b[\s\S]{0,36}\b(wallet|vcard|vcf)\b/i,
  /\b(wallet|vcard)\b[\s\S]{0,36}\b(directory|listing|contents?)\b/i,
  /\b(expose|reveal|dump|leak)\b[\s\S]{0,36}\b(wallet|vcard|vcf|pkpass)\b/i,
  /\b(retrieve|fetch|download|get)\b[\s\S]{0,36}\b(\.vcf|vcard|pkpass)\b/i,
...
const PRIVATE_PATH_REF =
  /(?:\/?Thomas-Gollogly\.(?:vcf|pkpass|CV\.pdf)|Thomas-Gollogly-CV\.pdf|\/?card\.html|\/?cv\.html|\/?wallet\/|contact\.json)/i;
