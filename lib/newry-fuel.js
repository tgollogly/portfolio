...
export const FUEL_MEMORY_VERSION = 2;
export const FUEL_MEMORY_PRICE_BASIS = "typical_area_v1";
...
/** Prefer the median of displayed NI quotes; use API average only if none exist. */
export function typicalHeatingPrice(niQuotePpls, apiAveragePpl, cheapestPpl) {
  const quotes = (niQuotePpls || []).map(Number).filter(Number.isFinite);
  if (quotes.length) return typicalPrice(quotes);
  if (Number.isFinite(Number(apiAveragePpl))) return Number(apiAveragePpl);
  return Number.isFinite(Number(cheapestPpl)) ? Number(cheapestPpl) : null;
}

/**
 * Smooth a forecast path with a 3-point median.
 * A median is robust when an outlier is adjacent to another flagged point,
 * whereas the previous neighbour-filter could accidentally preserve the spike.
 */
export function smoothForecastSeries(forecast) {
  const pts = (forecast || []).filter((f) => Number.isFinite(Number(f.estimate)));
  return pts.map((f, i) => {
    if (!pts[i - 1] || !pts[i + 1]) {
      return { ...f, typical: Math.round(Number(f.estimate) * 10) / 10 };
    }
    const vals = [
      Number(pts[i - 1].estimate),
      Number(f.estimate),
      Number(pts[i + 1].estimate),
    ].sort((a, b) => a - b);
    const typical = vals[1];
...
export function analyzeBuySignal({
  ...
  // buildFuelResponse stores today's typical price before analysis. Do not
  // weight it twice when it is already the final daily observation.
  if (!prices.length || Math.abs(prices[prices.length - 1] - current) > 0.001) {
    prices.push(current);
  }
  const ma7 = movingAverage(prices, 7);
  const ma30 = movingAverage(prices, 30);
  const ma7Days = Math.min(prices.length, 7);
  const ma30Days = Math.min(prices.length, 30);
  ...
  if (newsSentiment?.bias === "falling" && (verdict === "buy" || verdict === "watch")) {
    ...
    reasons.push("News points to rising energy costs — waiting is risky unless a clear dip appears.");
...
function buildGuideSummary({
  ...
  if (savingsVsAvgPpl != null && savingsVsAvgPpl < -1 && (action === "wait" || action === "wait_for_dip" || action === "watch")) {
    return kind === "diesel"
      ? `Today's ${formatPence(current)}/L is ${formatPence(Math.abs(savingsVsAvgPpl))}/L above the recent average — no cheaper dip is forecast; fill before prices rise further.`
      : `Today's ${formatPence(current)}/L is ${formatPence(Math.abs(savingsVsAvgPpl))}/L above the recent average — no cheaper dip is forecast; order before prices rise further.`;
  }
  ...
  if (action === "wait" || action === "wait_for_dip" || action === "watch") {
    return kind === "diesel"
      ? `${formatPence(current)}/L looks high, and no cheaper dip is forecast — fill before prices rise further.`
      : `${formatPence(current)}/L looks high, and no cheaper dip is forecast — order before prices rise further.`;
  }
...
export function buildMomSummary({
  ...
  if (verdict === "wait") {
    ...
    const orderSoon =
      kind === "diesel"
        ? "Fill before prices rise further — no cheaper dip is forecast."
        : "Order before prices rise further — no cheaper dip is forecast.";
    return {
      action: "ORDER SOON",
      emoji: "⚠️",
      oneLiner:
        firstSentence(guide?.summary) ||
        `Price is high with no cheaper dip forecast — order before prices rise further.`,
      short: "Before prices rise",
      doNow: [orderSoon, callHint],
      dontDo: ["Don't wait weeks hoping to save — forecast is flat or rising"],
      loading: false,
    };
  }
  ...
  if (verdict === "watch") {
    ...
    return {
      ...
      emoji: up ? "📈" : "👀",
      oneLiner: up
        ? `${formatPence(current)}/L is not cheap, and the short-term trend is up — fill before prices rise further.`
        : firstSentence(guide?.summary) || `Keep an eye on ${fuel} — a better window may open soon.`,
      short: up ? "Before prices rise" : "Worth watching",
      doNow: up
        ? [
            kind === "diesel"
              ? "Fill before prices rise further — trend is up."
              : "Order before prices rise further — trend is up.",
            "Check this page again in 2–3 days",
          ]
...
export function buildHoldOutlook({
  ...
  const summary = hasDip
    ...
    : kind === "diesel"
      ? `No cheaper dip forecast — fill before prices rise further.`
      : `No cheaper dip forecast — order before prices rise further.`;
...
export function buildWhenToBuyTimeline({
  ...
  const windows = future.map((f) => {
    ...
    let action = "hold";
    let note = "Typical smoothed forecast path — not a one-day low";
    if (vsNow <= -1.5) {
      action = "wait";
...
export function buildBuyGuide({
  ...
  const smoothExtPoints = smoothForecastSeries(extPoints);
  const nearTermDip = pickNearTermDip(current, extPoints);
  const nearTermBest = pickNearTermBest(extPoints);
  const hasNearTermDip = !!nearTermDip;
  const longTermBest = smoothExtPoints.filter((f) => f.day > 14).reduce(
    (best, f) => (!best || forecastScore(f) < forecastScore(best) ? f : best),
    null
  );
  ...
  if (verdict === "buy") {
    ...
    targetPrice = nearTermDip
      ? round(forecastScore(nearTermDip))
      : round(current);
    ...
    action = "wait";
  } else if (trend.slope < -0.08 && forecastMin.day > 0 && isMeaningfulDip(current, forecastScore(forecastMin))) {
    targetPrice = round(nearTermDip ? forecastScore(nearTermDip) : forecastScore(forecastMin));
    ...
    targetPrice = round(
      nearTermDip ? forecastScore(nearTermDip) : Math.min(current, forecastScore(forecastMin))
    );
  ...
  const stretchTarget =
    longTermBest &&
    forecastScore(longTermBest) < current - 0.5 &&
    longTermBest.day > (whenToBuy?.bestWindow?.day || nearTermDip?.day || 0)
      ? round(forecastScore(longTermBest))
      : null;
  ...
  return {
    ...
    forecastLowPpl: round(forecastScore(forecastMin)),
    forecastHighPpl: round(forecastScore(forecastMax)),
...
export async function fetchHeatingOil() {
  ...
  const niQuotePpls = (liveComparison.quotes || [])
    .filter((q) => q.region === "ni")
    .map((q) => q.pplGbp)
    .filter(Number.isFinite);
  const typicalPpl = typicalHeatingPrice(
    niQuotePpls,
    ni.ok ? ni.averagePpl : null,
    cheapestPpl
  );
  return {
    ok: true,
...
export function createEmptyFuelMemory() {
  ...
  return {
    version: FUEL_MEMORY_VERSION,
    priceBasis: FUEL_MEMORY_PRICE_BASIS,
    updatedAt: null,
...
/**
 * Cheapest-quote history cannot be mixed with typical-area history. Reset the
 * old v1 store once, otherwise a switch from 107.2p cheapest to 111.6p typical
 * is falsely reported as a 4.4p weekly rise.
 */
export function normalizeFuelMemoryBasis(memory) {
  if (
    memory?.version === FUEL_MEMORY_VERSION &&
    memory?.priceBasis === FUEL_MEMORY_PRICE_BASIS
  ) {
    return memory;
  }
  return createEmptyFuelMemory();
}
...
/** One typical-area close per calendar day for true day-based MAs/trends. */
export function flattenMemoryPrices(memory, kind) {
  ...
  for (const d of daily) {
    if (!d?.date || !Number.isFinite(Number(d.close))) continue;
    byDate.set(d.date, {
      price: Number(d.close),
      date: d.date,
      at: `${d.date}T23:59:59Z`,
    });
  }
  for (const r of [...recent].sort((a, b) => String(a.at).localeCompare(String(b.at)))) {
    if (!r?.date || !Number.isFinite(Number(r.price))) continue;
    byDate.set(r.date, { price: Number(r.price), date: r.date, at: r.at });
  }
...
export function recordFuelPrediction(memory, kind, signal) {
  ...
  const last = [...preds].reverse().find((p) => p.kind === kind);
  const sixHours = 6 * 60 * 60 * 1000;
  if (
    last &&
    last.verdict === signal.verdict &&
    Math.abs(Number(last.current) - Number(signal.current)) < 0.05 &&
    Date.now() - Date.parse(last.at) < sixHours
  ) {
    return mem;
  }
  preds.push({
...
/** Trend line from stored typical-area readings plus the current price. */
export function buildLiveTrendSeries(govSeries, kvHistory, livePrice, kind, liveDieselPpl) {
  ...
  for (const k of kv) {
    points.push({
      ...
      price: k.price,
      type: kind === "diesel" ? "local_pump" : "live",
      date: k.date || k.at?.slice(0, 10) || null,
      at: k.at || (k.date ? `${k.date}T12:00:00Z` : null),
    });
  }

  if (livePrice != null) {
    const now = new Date();
    const nowLabel = `Now · ${now.toLocaleDateString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`;
    points.push({
      label: nowLabel,
      price: livePrice,
      type: "now",
...
export function buildTrendSnapshot(series) {
  ...
  const local = series.filter(
    (p) => p.type !== "estimated_weekly" && Number.isFinite(Number(p.price))
  );
  const rows = local.length ? local : series.filter((p) => Number.isFinite(Number(p.price)));
  if (!rows.length) return null;
  const latest = rows[rows.length - 1];
  const now = Number(latest.price);
  const prev = rows.length > 1 ? Number(rows[rows.length - 2].price) : now;
  const latestMs = Date.parse(latest.at || (latest.date ? `${latest.date}T12:00:00Z` : ""));
  const dated = rows
    .map((p) => ({
      ...p,
      ms: Date.parse(p.at || (p.date ? `${p.date}T12:00:00Z` : "")),
    }))
    .filter((p) => Number.isFinite(p.ms) && p.ms < latestMs);
  const closestAtAge = (days, tolerance) => {
    const target = days * 24 * 60 * 60 * 1000;
    return dated
      .map((p) => ({ p, distance: Math.abs(latestMs - p.ms - target) }))
      .filter((x) => x.distance <= tolerance * 24 * 60 * 60 * 1000)
      .sort((a, b) => a.distance - b.distance)[0]?.p || null;
  };
  const hasDates = Number.isFinite(latestMs) && dated.length > 0;
  const weekAgo = hasDates ? closestAtAge(7, 2) : rows.at(-3);
  const monthAgo = hasDates ? closestAtAge(30, 5) : rows.at(-6);
  const changeRecent = Math.round((now - prev) * 10) / 10;
  const change7d = weekAgo ? Math.round((now - Number(weekAgo.price)) * 10) / 10 : null;
  const change30d = monthAgo ? Math.round((now - Number(monthAgo.price)) * 10) / 10 : null;
  const insufficientHistory = change7d == null;
  let direction = "flat";
  if (change7d != null && change7d > 0.4) direction = "up";
  else if (change7d != null && change7d < -0.4) direction = "down";
  const arrow = direction === "up" ? "↑" : direction === "down" ? "↓" : "→";
  const label = insufficientHistory
    ? "Weekly trend: building 7 days of typical-price history"
    : direction === "up"
...
export function buildAdviceTrendSnapshot(snapshot, signal) {
  ...
  if (snapshot.insufficientHistory) {
    const direction = prediction.direction;
    const arrow = prediction.arrow;
    return {
      ...snapshot,
      direction,
      arrow,
      label:
        direction === "steady"
          ? `7-day forecast: STEADY ${arrow} (weekly typical-price history still building)`
          : `7-day forecast: GOING ${direction.toUpperCase()} ${arrow} (${fcChange >= 0 ? "+" : "−"}${formatPence(Math.abs(fcChange))}/L)`,
    };
  }
  ...
  if (fcDir === "up" && !histUp) {
    ...
    return {
      ...
      direction: "up",
      arrow,
      label: `Outlook: GOING UP ${arrow} (+${formatPence(Math.abs(fcChange))}/L forecast over 7 days; weekly trend was down)`,
    };
  }
  ...
  if (fcDir === "steady" && histDown) {
    ...
    return {
      ...
      direction: "flat",
      arrow,
      label: `Outlook: STEADY ${arrow} (weekly trend was down; forecast flat ahead)`,
    };
  }
...
export function buildPredictionOutlook(signal) {
  ...
  const smoothed = smoothForecastSeries(signal.forecast);
  const day7 = smoothed[smoothed.length - 1];
  const day7Price = forecastScore(day7);
  const change = Math.round((day7Price - signal.current) * 10) / 10;
  ...
  const headline =
    direction === "up"
      ? `7-day prediction: GO UP ${arrow} to ~${formatPence(day7Price)}/L (+${formatPence(change)})`
      : direction === "down"
        ? `7-day prediction: GO DOWN ${arrow} to ~${formatPence(day7Price)}/L (−${formatPence(Math.abs(change))})`
        : `7-day prediction: STEADY ${arrow} around ${formatPence(signal.current)}/L`;
  const detail = `Today ${formatPence(signal.current)}/L → day ${day7.day}: ~${formatPence(day7Price)}/L`;
  return {
    direction,
    arrow,
...
export async function buildFuelResponse(env) {
  ...
  // v1/legacy history recorded cheapest quotes. It must not be blended with
  // v2 typical-area prices, so migrate by starting a clean price-basis series.
  memory = normalizeFuelMemoryBasis(memory);
...
export function buildDieselHoldExplain(signal, dieselContext = null) {
  ...
  let lead = firmHold
    ? `Typical pump price near ${dieselContext?.locationName || HOME.name} is ${formatPence(signal.current)}/L — high vs recent readings, but a dip is forecast if you can wait.`
    : `Typical pump price near ${dieselContext?.locationName || HOME.name} is ${formatPence(signal.current)}/L — high vs recent readings, but no dip is forecast. Waiting is risky.`;
