...
export function buildMomSummary({ verdict, headline, guide, holdOutlook, kind = "heating", current }) {
  ...
  if (verdict === "watch") {
    return {
      action: "WATCH",
      emoji: "👀",
      oneLiner: firstSentence(guide?.summary) || `Keep an eye on ${fuel} — a better window may open soon.`,
      short: "Worth watching",
      doNow: ["Check this page every few days", tankOk],
      dontDo: ["Don't panic-buy — price is OK but not a bargain"],
      loading: false,
    };
  }
