...
/** On-page buy flash — no Slack/push setup required. */
export function buildBuyAlertState({ alertWorthy, signals }) {
  const items = [];
  const h = signals?.heating;
  const d = signals?.diesel;
  if (h?.verdict === "buy" && h.current != null) {
    items.push({
      kind: "heating",
      label: "Heating oil",
      pricePpl: h.current,
  ...
  if (d?.verdict === "buy" && d.current != null) {
    items.push({
      kind: "diesel",
      label: "Diesel",
      pricePpl: d.current,
  ...
  const message = active
    ? `BUY NOW — ${items.map((i) => `${i.label} ${formatPence(i.pricePpl)}/L`).join(" · ")}`
    : null;
  return {
    active,
    ...
    message,
    detail: active
      ? "Green alert — good time to buy before prices rise. Tap Call Safe Fuels to order."
      : null,
  };
}
...
export function buildSharePreviewMeta(payload, origin, mode = "path") {
  ...
  return {
    ...
    description,
    ogImage,
    ogImageAlt: heatingStr && dieselStr
      ? `Newry Fuel Watch — heating oil ${heatingStr}, diesel ${dieselStr}`
      : "Newry Fuel Watch — live heating oil and diesel prices near Newry",
