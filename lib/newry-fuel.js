...
/** Near-term forecast low only when it is a meaningful dip below today's price. */
export function pickNearTermDip(current, extendedForecast, maxDay = 14) {
  ...
  const nearTerm = future.filter((f) => f.day <= maxDay);
  const pool = nearTerm.length ? nearTerm : future.filter((f) => f.day <= 30);
  const search = (pool.length ? pool : future).filter((f) => isMeaningfulDip(current, f.estimate));
...
/** Plain-English one-liner for non-technical readers. */
export function buildMomSummary({ verdict, headline, guide, holdOutlook, kind = "heating", current }) {
  const fuel = kind === "diesel" ? "diesel" : "heating oil";
  const fillIfLow =
    kind === "diesel"
      ? "Fill up now if your tank is nearly empty."
      : "Order now if your tank is running low.";

  if (verdict === "buy") {
    return {
      action: "BUY NOW",
      emoji: "✅",
      oneLiner: guide?.summary
        ? `${guide.summary.split(".")[0]}.`
        : `Good time for ${fuel} — ${formatPence(current)}/L looks fair today.`,
      short: "Buy today",
    };
  }
  ...
  if (verdict === "wait") {
    if (guide?.hasNearTermDip && guide.savingsVsNowPpl >= 0.5) {
      ...
      return {
        action: "WAIT",
        emoji: "⏳",
        oneLiner: `Prices look high — wait about ${dayWord} if you can. Aim for ~${formatPence(guide.targetPricePpl)}/L.`,
        short: `Wait ~${dayWord}`,
      };
    }
    return {
      action: "FILL IF LOW",
      emoji: "⚠️",
      oneLiner: headline || `No cheaper dip coming soon. ${fillIfLow}`,
      short: "Don't wait too long",
    };
  }
  ...
  if (verdict === "watch") {
    return {
      action: "WATCH",
      emoji: "👀",
      oneLiner: guide?.summary || headline || `Keep an eye on ${fuel} — a better window may open soon.`,
  ...
  return {
    action: "CHECKING",
    emoji: "🔄",
    oneLiner: headline || "Waiting for live prices…",
    short: "Loading",
  };
}
...
export async function enrichHeatingSuppliersWithPhones(suppliers, origin, userAgent) {
  ...
  const results = await Promise.allSettled(
    list.map((s) => fetchDistributorPhone(origin, s.distributorPath, userAgent))
  );
  return list.map((s, i) => {
