...
export function buildHoldOutlook({
  ...
  const headline =
    kind === "diesel"
      ? hasDip
        ? `Hold off filling up ${dayLabel} — target ~${formatPence(dip.estimate)}/L at the pump`
        : `Hold off and recheck in ~1–7 days — no cheaper dip forecast yet`
      : hasDip
        ? `Hold ${dayLabel} — target ~${formatPence(dip.estimate)}/L (${formatPence(savings)} cheaper)`
        : `Hold ~1–7 days and recheck — no cheaper dip forecast yet`;
...
export function buildWhenToBuyTimeline({
  ...
  if (verdict === "buy") {
    ...
    buyBeforeCrowd = hasNearTermDip
      ...
      : `Today's ${formatPence(current)}/L looks high, but no cheaper dip is forecast soon — hold only if you can wait and recheck.`;
...
export function buildVerdictWhy(signal) {
  ...
  if (
    ...
    reasons.push("No cheaper dip forecast in the next 2 weeks — hold only if you can wait and recheck");
  }
