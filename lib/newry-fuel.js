...
function buildGuideSummary({
  ...
  return kind === "diesel"
    ? `Today's ${formatPence(current)}/L looks high — no clear dip in the next 2 weeks. Fill now if your tank is nearly empty.`
    : `Today's ${formatPence(current)}/L looks high — no clear dip in the next 2 weeks. Don't wait if your tank is nearly empty.`;
}
...
export function buildHoldOutlook({
  ...
  const best = pickNearTermBest(extendedForecast);
  if (!best) return null;
  ...
  const headline =
    kind === "diesel"
      ? hasDip
        ? `Hold off filling up ${dayLabel} — target ~${formatPence(best.estimate)}/L at the pump`
        : `Hold off filling up ${dayLabel} and recheck — no sharp dip forecast yet`
      : hasDip
  ...
  const summary =
    confidence === "high"
      ? kind === "diesel" && save50 > 0
        ? `Best guess: dip around ${best.dateLabel} (~${formatPence(best.estimate)}/L) — could save ~£${save50.toFixed(2)} on a 50L fill vs today.`
        : `Short-term dip around ${best.dateLabel} is the best guess (fairly reliable for 1–7 days).`
      : confidence === "medium"
        ? kind === "diesel"
          ? `Reasonable estimate — recheck every few days before you fill up; pump prices update live.`
          : `Reasonable estimate — recheck every few days; hold time updates with live prices.`
        : kind === "diesel"
          ? `Rough guide only — pump prices can jump overnight. Fill now if your tank is nearly empty.`
...
export function buildWhenToBuyTimeline({
  ...
  const bestFuture = nearTermBest || future.reduce((a, b) => (a.estimate <= b.estimate ? a : b));
  ...
  } else if (longTermBest.day > bestFuture.day && longTermBest.estimate < bestFuture.estimate * 0.99) {
...
export function buildBuyGuide({
  ...
  if (verdict === "buy") {
    ...
    targetPrice = nearTermBest
      ? round(nearTermBest.estimate)
      : round(Math.min(recentLow, forecastMin.estimate, ma30 ? ma30 * 0.95 : recentLow));
    timingShort = nearTermBest?.day ? `Hold ~${nearTermBest.day}d` : "Wait";
    timing = nearTermBest?.day
      ? `Hold ~${nearTermBest.day} day${nearTermBest.day > 1 ? "s" : ""} — target ~${formatPence(targetPrice)}/L`
      : "Wait — price looks high right now";
    action = "wait";
  } else if (trend.slope < -0.08 && forecastMin.day > 0 && forecastMin.estimate < current * 0.995) {
    targetPrice = round(nearTermBest?.estimate ?? forecastMin.estimate);
    const dipDay = nearTermBest?.day ?? forecastMin.day;
    timingShort = `Wait ~${dipDay}d`;
    timing = `Wait about ${dipDay} day${dipDay > 1 ? "s" : ""} — trend still falling`;
    action = "wait_for_dip";
  } else if (verdict === "watch") {
    targetPrice = round(nearTermBest?.estimate ?? Math.min(current, forecastMin.estimate));
    timingShort = nearTermBest?.day ? `Watch ~${nearTermBest.day}d` : "Buy soon";
    timing =
      targetPrice < current * 0.995
        ? `Best window may be in ~${nearTermBest?.day ?? forecastMin.day} days (~${formatPence(targetPrice)}/L)`
  ...
  const finalTarget =
    whenToBuy?.bestWindow?.pricePpl != null && action !== "buy_now"
  ...
  const stretchTarget =
    longTermBest &&
    longTermBest.estimate < finalTarget - 0.5 &&
    longTermBest.day > (whenToBuy?.bestWindow?.day || nearTermBest?.day || 0)
      ? round(longTermBest.estimate)
      : null;
  const stretchSavings = stretchTarget != null ? round(Math.max(0, current - stretchTarget)) : 0;
  const nearTermDay = whenToBuy?.bestWindow?.day ?? nearTermBest?.day ?? 0;
  const nearTermDateLabel = whenToBuy?.bestWindow?.dateLabel ?? nearTermBest?.dateLabel ?? null;
...
export async function fetchDieselNearby() {
  try {
    ...
    return {
      ...
      attribution: "Fuel Near You (fuelnearyou.com) — CC BY 4.0 · pump prices for standard road diesel",
    };
  } catch (e) {
    return { ok: false, error: String(e.message || e) };
  }
...
export function buildDieselDisplayMeta(diesel, govSeries = null) {
  ...
  return {
    ...
    postcode: diesel.postcode || NEWRY.postcode,
    ...
    headline: `Cheapest pump price within ${diesel.searchRadiusMiles || 12} miles of ${diesel.postcode || "BT35"}`,
    subtitle: station
...
export function buildDieselHoldExplain(signal, dieselContext = null) {
  ...
  let lead = `Pump price at ${station} is ${formatPence(signal.current)}/L — that's high vs recent local readings, so waiting may be worth it if you can.`;
  ...
  const detailParts = [];
  if (ho?.headline) detailParts.push(ho.headline);
  if (target != null && savePpl > 0.3) {
    detailParts.push(
