...
export function analyzeNewsSentiment(headlines) {
  ...
  for (const raw of headlines || []) {
    ...
    relevant.push(h);
    if (RISE_NEWS.test(h)) rise++;
    if (FALL_NEWS.test(h)) fall++;
  }
  let bias = "neutral";
  if (rise > fall) bias = "rising";
  else if (fall > rise) bias = "falling";
  const summary =
    bias === "rising"
      ? "News suggests pump prices may rise — buying before headlines spread could help."
      : bias === "falling"
        ? "News suggests prices may soften — waiting could save a little more."
        : "No strong fuel-price direction in recent headlines.";
  return { bias, rise, fall, relevant: relevant.slice(0, 8), summary };
}
