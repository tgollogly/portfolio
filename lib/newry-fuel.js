...
/** Top banner — buy (green), hold (red), or watch (amber). Static, matches verdict. */
export function buildBuyAlertState({ alertWorthy, signals }) {
  ...
  if (mode === "buy") {
    ...
    detail = "Good time to buy before prices rise.";
  } else if (mode === "hold") {
    message = "Hold if you can — prices look high";
    detail = priceLine(items) + " — wait for a dip if you can.";
  } else if (mode === "watch") {
    message = "Worth watching — prices may improve soon";
    detail = priceLine(items);
  }
