...
export function analyzeBuySignal({
  ...
  const forecast = buildShortForecast(recent, trend);
...
/** 7-day linear forecast from recent readings. */
export function buildShortForecast(recent, trend) {
  const forecast = [];
  for (let i = 1; i <= 7; i++) {
    forecast.push({
      day: i,
      estimate: Math.max(
        0,
        Math.round((trend.intercept + trend.slope * (recent.length + i)) * 100) / 100
      ),
    });
  }
  return forecast;
...
/** Estimate price at day 1 … 180 — short trend + UK seasonal diesel patterns. */
export function buildExtendedForecast({ current, recent, trend, govSeries, kind = "heating" }) {
  ...
  return days.map((day) => {
    ...
    const trendEst = current + trend.slope * day * damp;
    ...
    let blend;
    if (day <= 7) blend = trendEst * 0.85 + seasonalEst * 0.15;
    else if (day <= 30) blend = trendEst * 0.45 + seasonalEst * 0.55;
...
export function buildWhenToBuyTimeline({
  ...
  if (verdict === "buy" || (tomorrow && tomorrow.estimate >= current * 0.998)) {
...
export function buildBuyGuide({
  ...
  if (whenToBuy?.headline && action !== "buy_now") {
    timing = whenToBuy.headline;
    timingShort = whenToBuy.bestWindow.day === 0
      ? "Buy now"
      : whenToBuy.bestWindow.day === 1
        ? "Buy tomorrow"
        : `Wait ~${whenToBuy.bestWindow.day}d`;
  } else if (whenToBuy?.headline && action === "buy_now") {
    timing = whenToBuy.headline;
  }
