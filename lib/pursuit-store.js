...
export const PURSUIT_REFRESH_LOCK = "pursuit:refresh-lock";
export const PURSUIT_BG_INTERVAL_MS = 12 * 60 * 60 * 1000;
...
function parseAiQuestions(text) {
  ...
  for (let i = 0; i < parsed.length; i++) {
    ...
    const textQ = String(q.q || q.question || "").trim();
    if (!textQ || opts.length !== 4 || ans < 0 || ans > 3) continue;
    out.push({
      id: `ai${now}${i}`,
      d: diff,
      q: textQ.slice(0, 280),
...
/** Fallback when cron is not registered — refresh while players use the game. */
export async function maybeBackgroundPursuitRefresh(env, requestUrl, { geminiFn, getKeyFn } = {}) {
  if (!env.PURSUIT_DB) return;
  const stats = await getPursuitStats(env);
  const last = stats.lastRefresh || 0;
  if (Date.now() - last < PURSUIT_BG_INTERVAL_MS) return;

  if (env.PURSUIT_KV) {
    const locked = await env.PURSUIT_KV.get(PURSUIT_REFRESH_LOCK);
    if (locked && Date.now() - Number(locked) < 3_600_000) return;
    await env.PURSUIT_KV.put(PURSUIT_REFRESH_LOCK, String(Date.now()), { expirationTtl: 3600 });
  }

  await ensureSchema(env);
  const count = await getQuestionCount(env);
  if (count === 0 && requestUrl) {
    // seed handled by ensurePursuitBank callers; skip duplicate work here
    return;
  }

  const key = getKeyFn ? await getKeyFn(env) : null;
  if (key && geminiFn) {
    try {
      await refreshQuestionsWithAi(env, geminiFn, key);
    } catch {
      /* non-fatal */
    }
  }
  if (count < 10_000_000) {
    await expandProceduralQuestions(env, Math.min(count + 500, 10_000_000));
  }
}
