/** Pursuit quiz — D1 question bank, KV player memory, optional AI refresh. */
...
export async function getPursuitStats(env) {
  ...
  if (env.PURSUIT_DB && total > 0) {
    ...
    const meta = await env.PURSUIT_DB.prepare(
      "SELECT key, value FROM pursuit_meta WHERE key IN ('last_ai_refresh', 'last_feed_refresh')"
    ).all();
    for (const row of meta.results || []) {
      if (row.key === "last_ai_refresh") lastRefresh = Number(row.value) || null;
      if (row.key === "last_feed_refresh") lastFeedRefresh = Number(row.value) || null;
    }
  }
  return { total, bySource, lastRefresh, lastFeedRefresh, scalable: true };
}
...
export async function resetLeaderboard(env) {
  if (!env.PURSUIT_KV) return { ok: false, error: "no kv" };
  const previous = await getLeaderboard(env);
  await env.PURSUIT_KV.put(PURSUIT_LB_KEY, JSON.stringify([]));
  const now = Date.now();
  await env.PURSUIT_KV.put(
    "pursuit:leaderboard_reset_at",
    JSON.stringify({ at: now, cleared: previous.length })
  );
  return { ok: true, cleared: previous.length, at: now };
}
  ...
  if (env.PURSUIT_KV) {
    try {
      ...
      if (raw) {
        ...
        const lines = [];
        for (const [feed, titles] of Object.entries(cached.feeds || {})) {
          for (const t of (titles || []).slice(0, 3)) lines.push(`- [${feed}] ${t}`);
        }
        if (lines.length) headlineHint = `\nUse these recent headlines for 30% of questions (facts only, no opinion):\n${lines.slice(0, 12).join("\n")}\n`;
      }
    } catch {
...
Include 40% about recent public facts and current events from ${month} (sport, politics, science, UK/Ireland news — only well-established public facts, no gossip).${headlineHint}
...
All four options must be unique strings. Return ONLY valid JSON array, no markdown.`;
