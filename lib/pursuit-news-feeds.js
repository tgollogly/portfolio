...
export async function fetchOpenTdb(amount, fetchFn = fetch) {
  const target = Math.max(1, amount);
  const now = Date.now();
  const out = [];
  let remaining = target;
  let batchIdx = 0;
  while (remaining > 0 && out.length < target) {
    const batch = Math.min(50, remaining);
    const url = `${OPENTDB_URL}?amount=${batch}&type=multiple`;
    ...
    for (const item of data.results) {
      const mapped = mapOpenTdbItem(item, batchIdx++, now);
...
export async function generateFeedQuestions(fetchFn = fetch, opts = {}) {
  ...
  const otdbAmount = opts.otdbAmount ?? 80;
  try {
  ...
  for (const feed of RSS_FEEDS) {
    try {
      const headlines = await fetchRssHeadlines(feed.url, fetchFn, 15);
      headlinesByFeed[feed.id] = headlines.slice(0, 10);
      all.push(...questionsFromHeadlines(headlines, feed.id, now, 8));
    } catch (e) {
