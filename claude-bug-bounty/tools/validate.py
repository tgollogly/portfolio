...
def gate4_not_dup(vuln_type: str, endpoint: str, program_handle: str, auto_dedup: bool = False) -> tuple[bool, dict]:
    gate_header(4, "Is It a Dup?")
    print("  Check HackerOne disclosed reports, GitHub issues, and recent changelog.")
    print()

    novelty_result = None
    if auto_dedup and program_handle and (vuln_type or endpoint):
        try:
            from tools.novelty_engine import assess_novelty, format_result

            novelty_result = assess_novelty(
                program_handle=program_handle,
                endpoint=endpoint,
                vuln_class=vuln_type,
                title=f"{vuln_type} in {endpoint}",
            )
            print(format_result(novelty_result))
            print()
            if not novelty_result.safe_to_investigate:
                notes = {
                    "not_in_h1_disclosed": False,
                    "not_in_github_issues": False,
                    "checked_git_history": False,
                    "h1_similar_reports": [m.title for m in novelty_result.matches],
                    "novelty_score": novelty_result.novelty_score,
                    "rejection_reason": "duplicate_or_already_disclosed",
                }
                print(f"\n  {RED}GATE 4 FAIL: duplicate_or_already_disclosed (auto-dedup){RESET}")
                return False, notes
        except Exception as exc:
            print(f"  {YELLOW}Auto-dedup unavailable: {exc}{RESET}")
    ...
    if novelty_result is not None:
        notes["novelty_score"] = novelty_result.novelty_score
        notes["novelty_recommendation"] = novelty_result.recommendation
...
def main():
    ...
    parser.add_argument(
        "--auto-dedup",
        action="store_true",
        help="Run novelty_engine.py before Gate 4 and auto-fail on strong public duplicates",
    )
    parser.add_argument("--endpoint", default="", help="Endpoint for auto-dedup")
    parser.add_argument("--vuln-type", default="", help="Vulnerability type for auto-dedup")
    ...
    vuln_type      = args.vuln_type or ask("Vulnerability type (e.g., 'IDOR', 'Stored XSS', 'SSRF')")
    endpoint       = args.endpoint or ask("Affected endpoint (e.g., '/api/invoices/:id')")
    ...
    g4_pass, g4_notes = gate4_not_dup(
        vuln_type,
        endpoint,
        target_program,
