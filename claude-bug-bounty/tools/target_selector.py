...
def score_program(prog):
    ...
    if bounty_max >= 1000000:
        score += 40
    elif bounty_max >= 100000:
        score += 35
...
def select_targets(programs, top_n=10, min_bounty=0):
    """Score and rank programs, return top N."""
    if min_bounty > 0:
        before = len(programs)
        programs = [p for p in programs if (p.get("bounty_max") or 0) >= min_bounty]
        print(f"\n[*] Filtered to {len(programs)} programs with max bounty >= ${min_bounty:,} ({before - len(programs)} removed)")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Only include programs whose max bounty table value is at least this amount (USD)",
    )
    ...
    selected = select_targets(programs, top_n=args.top, min_bounty=args.min_bounty)
